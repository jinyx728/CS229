...
)
{
    ...
    {
	sym = (2 * (mfloat) nzboth) / ((mfloat) (nz - nzdiag)) ;
    }
	...
	Info [AMD_STATUS] = AMD_OK ;
	Info [AMD_N] = (mfloat)n ;
	Info [AMD_NZ] = (mfloat)nz ;
	Info [AMD_SYMMETRY] = sym ;					/* symmetry of pattern of A */
	Info [AMD_NZDIAG] = (mfloat)nzdiag ;	    /* nonzeros on diagonal of A */
	Info [AMD_NZ_A_PLUS_AT] = (mfloat)nzaat ;	/* nonzeros in A+A' */
    }
