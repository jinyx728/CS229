 ...
 * The int array Ai and the mfloat array Ax are of size nz.  This data structure
 ...
 * mfloat array Lx of the same size as Li).  It has a unit diagonal, which is
...
(
    ...
    LDL_int Ap [ ],	     /* input of size n+1, not modified */
    LDL_int Ai [ ],	     /* input of size nz=Ap[n], not modified */
    mfloat Ax [ ],	     /* input of size nz=Ap[n], not modified */
    ...
    mfloat eps,          /* input, regularization threshold */
	mfloat delta,        /* input, regularization parameter */
    LDL_int Lnz [ ],	 /* output of size n, not defn. on input */
    LDL_int Li [ ],	     /* output of size lnz=Lp[n], not defined on input */
    mfloat Lx [ ],	     /* output of size lnz=Lp[n], not defined on input */
    mfloat D [ ],	     /* output of size n, not defined on input */
    mfloat Y [ ],	     /* workspace of size n, not defn. on input or output */
    LDL_int Pattern [ ], /* workspace of size n, not defn. on input or output */
    LDL_int Flag [ ]	 /* workspace of size n, not defn. on input or output */
#if PROFILING > 1
    ,mfloat *t1,           /* time for non-zero pattern computation */
    mfloat *t2           /* time for back-solves */
#endif
)
{
    mfloat yi, l_ki;
...
(
    ...
    mfloat X [ ],	/* size n.  right-hand-side on input, soln. on output */
    LDL_int Lp [ ],	/* input of size n+1, not modified */
    LDL_int Li [ ],	/* input of size lnz=Lp[n], not modified */
    mfloat Lx [ ]	/* input of size lnz=Lp[n], not modified */
){
    LDL_int j, p, p2 ;
    for (j = 0 ; j < n ; j++){
		p2 = Lp [j+1] ;
...
(
    LDL_int n,		/* L is n-by-n, where n >= 0 */
	mfloat b [ ],	/* size n. input. right-hand-side */    
    LDL_int Lp [ ],	/* input of size n+1, not modified */
    LDL_int Li [ ],	/* input of size lnz=Lp[n], not modified */
    mfloat Lx [ ],	/* input of size lnz=Lp[n], not modified */
	mfloat x [ ]	/* size n. output. solution to Lx=b */
)
{
    LDL_int j, p, p2 ;
...
(
    ...
    mfloat X [ ],	/* size n.  right-hand-side on input, soln. on output */
    mfloat D [ ]	/* input of size n, not modified */
)
{
    LDL_int j ;
