...
void createKKT_U(spmat* Gt, spmat* At, cone* C, idxint** S, spmat** K,
                 idxint* AttoK, idxint* GttoK, idxint is_backward)
{
    ...
    idxint *Sign;
    idxint cone_strt_i=0;
    idxint cont_strt_block=0;
#ifdef EXPCONE
	...
	for( i=0; i<C->nsoc; i++ ){
        ...
        else if(is_backward==SYMMETRIZED_GRAD){
            nnzK+=2*C->soc[i].p+2;
        }
        else{
            nnzK += 3*C->soc[i].p+1;
    ...
    if(is_backward==FULL_SYM_GRAD){
        ...
        for( ks=n+p+m; ks < n+p+m+C->lpc->p; ks++){
        ...
        for( ks=n; ks < n+p; ks++){
            ...
            ks = n+p+C->lpc->p;
            for( l=0; l<C->nsoc; l++){
                if(is_backward==SYMMETRIZED_GRAD){
                    Sign[ks++] = +1;
...
// #endif
	...
	/* LP cone */
    /* not exactly sure what should be done for FULL_SYM_GRAD here*/
	i = 0;
	for( j=0; j < C->lpc->p; j++ ){
        /* copy in G' */
		row = Gt->jc[j];
    ...
    if(is_backward==FULL_SYM_GRAD){
        ...
        for(l=0;l<C->nsoc;l++){
            conesize=C->soc[l].p;
            cone_strt_i=n+p+m+cone_strt+1*l;
            for(j=0;j<conesize;j++){
                Kjc[cone_strt_i+j]=k;
                C->soc[l].Didx[j] = k; /* start of each colume */
                /* S */
                /* set to identity to start with */
                /* needs to save k ??*/
                if(j==0){
                    Kir[k]=n+p+cone_strt;
                    Kpr[k]=1; 
                ...
                if(j==0){
                    Kir[k]=cone_strt_i;
                    Kpr[k]=DELTASTAT; /* or 1+DELTASTAT ?? */
                    k++;
                }
                else{
                    Kir[k]=cone_strt_i+j;
            ...
            /* v */
            Kjc[cone_strt_i+conesize]=k;
            for(r=1;r<conesize;r++){
                Kir[k]=cone_strt_i+r;
                Kpr[k]=0;
                k++;
            }
            Kir[k]=cone_strt_i+conesize;
            Kpr[k]=1;
            k++;
        ...
        for( l=0; l < C->nsoc; l++ ){
            ...
            if(is_backward==SYMMETRIZED_GRAD){
                ...
                // dummy
                Kjc[cone_strt_i+conesize+1]=k;
                Kir[k]=cone_strt_i+conesize+1;
                Kpr[k]=+1;
                k++;
            }
            else{
...
#if CONEMODE == 0
    if(is_backward==SYMMETRIZED_GRAD){
        exp_cone_strt += C->nsoc; // not being used
    }
    else{
        exp_cone_strt += 2*C->nsoc; // not being used
    }
#endif
     for( l=0; l < C->nexc; l++ ){
...
pwork* ECOS_setup(idxint n, idxint m, idxint p, idxint l, idxint ncones, idxint* q, idxint nexc,
                   ...
                   pfloat* c, pfloat* h, pfloat* b,idxint is_backward)
{
  ...
  if(mywork->A) {
	...
	createKKT_U(Gt, At, mywork->C, &Sign, &KU, AttoK, GttoK, is_backward);
#if PROFILING > 1
