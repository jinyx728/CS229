...
idxint checkExitConditions(pwork* w, idxint mode)
{
    ...
    // printf("\ncheckExitConditions:feastol:%f,abstol:%f,reltol:%f\n",feastol,abstol,reltol);
    // printf("\ncheckExitConditions:%d,%d,%d\n",( -w->cx > 0 || -w->by - w->hz >= -abstol ),( w->info->pres < feastol && w->info->dres < feastol ),( w->info->gap < abstol || w->info->relgap < reltol  ));
...
#define QUASI_SOLVER 0
#define LU_SOLVER 1
#define BCG_SOLVER 2
#define QR_SOLVER 3
#define DENSE_LU_SOLVER 4

idxint m_solve(kkt* KKT, spmat* A, spmat* G, pfloat* Pb, pfloat* dx, pfloat* dy, pfloat* dz, idxint n, idxint p, idxint m, cone* C, idxint isinit, idxint nitref)
{
    idxint linear_solver=QUASI_SOLVER;
    // idxint linear_solver=LU_SOLVER;
    // idxint linear_solver=BCG_SOLVER;
    // idxint linear_solver=QR_SOLVER;
    // idxint linear_solver=DENSE_LU_SOLVER;
    if(linear_solver==LU_SOLVER){
        lu_solve(KKT, A, G, Pb, dx, dy, dz, n, p, m, C);
        // check
        pfloat *dx0=(pfloat *)MALLOC(n*sizeof(pfloat));
        pfloat *dy0=(pfloat *)MALLOC(p*sizeof(pfloat));
        pfloat *dz0=(pfloat *)MALLOC(m*sizeof(pfloat));
        kkt_solve(KKT, A, G, Pb, dx0, dy0, dz0, n, p, m, C, isinit, nitref);
        check_consistency(dx,dy,dz,dx0,dy0,dz0,n,p,m);
        return 0;
    }
    else if(linear_solver==BCG_SOLVER){
        bcg_solve(KKT, A, G, Pb, dx, dy, dz, n, p, m, C);
        // check
        pfloat *dx0=(pfloat *)MALLOC(n*sizeof(pfloat));
        pfloat *dy0=(pfloat *)MALLOC(p*sizeof(pfloat));
        pfloat *dz0=(pfloat *)MALLOC(m*sizeof(pfloat));
    ...
    else if(linear_solver==QR_SOLVER){
        qr_solve(KKT, A, G, Pb, dx, dy, dz, n, p, m, C);
        // check
        pfloat *dx0=(pfloat *)MALLOC(n*sizeof(pfloat));
        pfloat *dy0=(pfloat *)MALLOC(p*sizeof(pfloat));
        pfloat *dz0=(pfloat *)MALLOC(m*sizeof(pfloat));
    ...
    else if(linear_solver==DENSE_LU_SOLVER){
        dense_lu_solve(KKT, A, G, Pb, dx, dy, dz, n, p, m, C);
        // check
        pfloat *dx0=(pfloat *)MALLOC(n*sizeof(pfloat));
        pfloat *dy0=(pfloat *)MALLOC(p*sizeof(pfloat));
        pfloat *dz0=(pfloat *)MALLOC(m*sizeof(pfloat));
        ...
        return kkt_solve(KKT, A, G, Pb, dx, dy, dz, n, p, m, C, isinit, nitref);
    }
}
 ...
 */
idxint init(pwork* w,idxint is_backward)
{
    ...
    kkt_init(w->KKT->PKPt, w->KKT->PK, w->C, is_backward);
	...
	tic(&tkktsolve);
#endif
    w->info->nitref1 = m_solve(w->KKT, w->A, w->G, w->KKT->RHS1, w->KKT->dx1, w->KKT->dy1, w->KKT->dz1, w->n, w->p, w->m, w->C, 1, w->stgs->nitref);
	...
	tic(&tkktsolve);
#endif
	w->info->nitref2 = m_solve(w->KKT, w->A, w->G, w->KKT->RHS2, w->KKT->dx2, w->KKT->dy2, w->KKT->dz2, w->n, w->p, w->m, w->C, 1, w->stgs->nitref);
#if PROFILING > 1
	w->info->tkktsolve += toc(&tkktsolve);
#endif
...
#if PRINTLEVEL > 2
    PRINTTEXT("TAU=%6.4e  KAP=%6.4e  PINFRES=%6.4e  DINFRES=%6.4e\n",(double)w->tau,(double)w->kap,(double)info->pinfres, (double)info->dinfres );
#endif
...
#if PRINTLEVEL       > 2
                                 PRINTTEXT("Centrality violation %e, step %e , mu %e, barrier %e\n",(double)*centrality,(double)alpha,(double)mu,(double)barrier);
#endif
...
idxint ECOS_solve(pwork* w)
{
	...
	for( w->info->iter = 0; w->info->iter <= w->stgs->maxit ; w->info->iter++ ){
		...
		tic(&tkktsolve);
#endif
		w->info->nitref1 = m_solve(w->KKT, w->A, w->G, w->KKT->RHS1, w->KKT->dx1, w->KKT->dy1, w->KKT->dz1, w->n, w->p, w->m, w->C, 0, w->stgs->nitref);
#if PROFILING > 1
		w->info->tkktsolve += toc(&tkktsolve);
#endif
		...
		tic(&tkktsolve);
#endif
		w->info->nitref2 = m_solve(w->KKT, w->A, w->G, w->KKT->RHS2, w->KKT->dx2, w->KKT->dy2, w->KKT->dz2, w->n, w->p, w->m, w->C, 0, w->stgs->nitref);
#if PROFILING > 1
		w->info->tkktsolve += toc(&tkktsolve);
#endif
    ...
    PRINTTEXT("Backtrack affine\n");
		...
		tic(&tkktsolve);
#endif
		w->info->nitref3 = m_solve(w->KKT, w->A, w->G, w->KKT->RHS2, w->KKT->dx2, w->KKT->dy2, w->KKT->dz2, w->n, w->p, w->m, w->C, 0, w->stgs->nitref);
#if PROFILING > 1
		w->info->tkktsolve += toc(&tkktsolve);
#endif
...
#if PRINTLEVEL > 0
#if PROFILING > 0
	if( w->stgs->verbose ) PRINTTEXT("\nRuntime: %f seconds.", (double)(w->info->tsetup + w->info->tsolve));
#endif
	if( w->stgs->verbose ) PRINTTEXT("\n\n");
#endif
...
/* 
 * a wrapper of init function
 */
idxint ECOS_backward_init(pwork *w,idxint backward_mode)
{
    if(backward_mode==FULL_SYM_GRAD){
        return ECOS_OPTIMAL;
    }
    idxint initcode = init(w,backward_mode); // This will compute RHS1, which we do not need
    if( initcode == ECOS_FATAL ){
        if( w->stgs->verbose ) PRINTTEXT("\nFatal error during initialization, aborting.");
        return ECOS_FATAL;
    }
...
void symmetrized_grad_update(spmat* PKP,idxint *P,cone *C,pfloat *s,pfloat *z)
{
    idxint i, j, k, t, conesize,conesize_m1;
    pfloat gamma,gamma_inv,sqrt_gamma_inv,z0,s0,sqrt_z0,sqrt_s0,s0_over_z0,z0_over_s0,z1z1,s1s1;
    pfloat *z1,*s1,*st,*zt;
    pfloat sqrt2,factor;
    pfloat delta=DELTASTAT;
    // pfloat delta=0;
    // pfloat reg=100;
    sqrt2=sqrt(2);
    for( i=0; i < C->lpc->p; i++ ){ PKP->pr[P[C->lpc->kkt_idx[i]]] = -C->lpc->v[i] - delta; }
    t=C->lpc->p;
    for( i=0; i<C->nsoc;i++){
        st=s+t;zt=z+t;conesize=C->soc[i].p;conesize_m1=conesize-1;
        s0=*st;z0=*zt;s1=st+1;z1=zt+1;
        sqrt_s0=sqrt(s0);sqrt_z0=sqrt(z0);
        s0_over_z0=s0/z0;z0_over_s0=z0/s0;
        z1z1=0;
        for(j=0;j<conesize_m1;j++){
            z1z1+=z1[j]*z1[j];
        }
        gamma=z0*z0-z1z1;
        gamma_inv=1/gamma;
        sqrt_gamma_inv=1/sqrt(gamma);

        s1s1=0;
        for(j=0;j<conesize_m1;j++){
            s1s1+=s1[j]*s1[j];
        }
        // printf("z0:%f,s0:%f,gamma_inv:%f,sqrt_gamma_inv:%f,eta_sqr:%f\n",z0,s0,gamma_inv,sqrt_gamma_inv,sqrt((s0*s0-s1s1)/gamma));
        if(z0>=s0){
            /* D */
            PKP->pr[P[C->soc[i].Didx[0]]] = s0_over_z0 - delta;
            for (k=1; k < conesize; k++) {
                PKP->pr[P[C->soc[i].Didx[k]]] = -s0_over_z0 - delta;
            }
            factor=sqrt_gamma_inv*sqrt2*sqrt_s0;
            /* u */
            j=0;
            PKP->pr[P[C->soc[i].Didx[conesize_m1] + j++]] = -factor*sqrt_z0;
            // PKP->pr[P[C->soc[i].Didx[conesize_m1] + j++]] = 0;
            for (k=0; k < conesize_m1; k++) {
                // PKP->pr[P[C->soc[i].Didx[conesize_m1] + j++]] = -factor/MAX(sqrt_z0,10) * z1[k];
                // PKP->pr[P[C->soc[i].Didx[conesize_m1] + j++]] = 0;
                PKP->pr[P[C->soc[i].Didx[conesize_m1] + j++]] = factor/MAX(sqrt_z0,0.01) * z1[k];
            }
            PKP->pr[P[C->soc[i].Didx[conesize_m1] + j++]] = +1 + delta;
            ...
            factor=sqrt_gamma_inv*sqrt2*sqrt_z0;
            /* u */
            j=0;
            PKP->pr[P[C->soc[i].Didx[conesize_m1] + j++]] = -factor*sqrt_s0;
            for (k=0; k < conesize_m1; k++) {
                // PKP->pr[P[C->soc[i].Didx[conesize_m1] + j++]] = -factor/MAX(sqrt_s0,1) * s1[k];
                PKP->pr[P[C->soc[i].Didx[conesize_m1] + j++]] = -factor/MAX(sqrt_s0,0.01) * s1[k];
            }
            PKP->pr[P[C->soc[i].Didx[conesize_m1] + j++]] = +1 + delta;
...
void backward_kkt_update(spmat* PKP,idxint *P,cone *C,pfloat *s,pfloat *z)
{
    if(GRAD_MODE==FULL_SYM_GRAD){
        full_sym_grad_update(PKP,P,C,s,z);
    }
    else if(GRAD_MODE==SYMMETRIZED_GRAD){
        symmetrized_grad_update(PKP,P,C,s,z);
    }
    else{
        kkt_update(PKP,P,C);
    }
...
/* 
 * assuming g_in and g_out are allocated, but not permuted
 * g_in has length n
 * dz has length m
 * assuming g_in is already scaled 
 */
idxint ECOS_backward_prop(pwork *w,pfloat *g_in,pfloat *dx,pfloat *dy,pfloat *dz)
{
    idxint n=w->n,p=w->p,m=w->m;
    idxint nK=n+p+m;
    idxint *Pinv=w->KKT->Pinv;
    idxint i,j;
    ...
    stats* info = w->info;
    info->gap = eddot(w->m, w->s, w->z);
    info->mu = (info->gap + w->kap*w->tau) / (w->D + 1); // mu is not usefull, but it not a bad idea to verify gap and mu

    if( updateScalings(w->C, w->s, w->z, w->lambda, info->mu) == OUTSIDE_CONE ){ 
        PRINTTEXT("backprop:OUTSIDE_CONE\n");
        return ECOS_OUTCONE;
    }

    backward_kkt_update(w->KKT->PKPt,w->KKT->PK,w->C,w->s,w->z);
    // idxint KKT_FACTOR_RETURN_CODE = kkt_factor(w->KKT, w->stgs->eps, w->stgs->delta);
    idxint KKT_FACTOR_RETURN_CODE = kkt_factor(w->KKT, w->stgs->delta, w->stgs->delta);
    if(KKT_FACTOR_RETURN_CODE!=KKT_OK){
        PRINTTEXT("backprop:kkt_factor wrong\n");
    }


    if(GRAD_MODE==FULL_SYM_GRAD){
        nK+=m+w->C->nsoc;
        memset(w->KKT->RHS1,0,nK*sizeof(pfloat));
        j=0;
        for(i=0; i<n; i++){ w->KKT->RHS1[Pinv[j++]]=g_in[i]; }
        full_sym_grad_solve(w->KKT, w->A, w->G, w->KKT->RHS1, dx, dy, dz, w->n, w->p, w->m, w->C, 0,  0/*nitref*/);
    }
    else{
    #if CONEMODE == 0
        nK += 2*w->C->nsoc;    
        ...
        for(i=0; i<n; i++){ w->KKT->RHS1[Pinv[j++]]=g_in[i]; }
        // idxint nitref
        m_solve(w->KKT, w->A, w->G, w->KKT->RHS1, dx, dy, dz, w->n, w->p, w->m, w->C, 0,  0/*nitref*/);
    }
