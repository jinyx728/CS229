...
def displacement_gen(vt1file, vt2file):
    ...
    for i in range(len(vt1)):
        line1 = vt1[i].split(" ")
        line2 = vt2[i].split(" ")
        print("vt {} {}".format(float(line1[0])+float(line2[1]), float(line1[1])+float(line2[2])))

    return

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("-d", "--displacement", help="coordinate displacement", required=True)
    parser.add_argument("-g", "--groundtruth", help="ground truth texture", required=True)
    args = parser.parse_args()
    displacement_gen(args.displacement, args.groundtruth)